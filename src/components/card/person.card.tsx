...
<Image alt='ads' className='h-32 p-3 w-60 m-auto pb-8' height={1000} 
quality={100} src={section} 
width={1000}/>
   </SwiperSlide>))}
          </Swiper>
...
{data.map((section)=>(
 <div key={section.id} className="w-full my-4 max-w-[300px] col-span-4 lg:col-span-1 p-8 rounded-xl text-gray-800  overflow-hidden group 
 hover:shadow-2xl hover:shadow-sky-500/50 motion-safe:transition-all motion-safe:duration-700">
<figure className="relative w-40 h-40 m-0 mx-auto rounded-full outline outline-offset-4 outline-sky-500 
...
<Image alt="emolyer job" className="rounded-full block w-[200px] h-[160px] bg-sky-500 object-cover z-10 relative" height={500} quality={80} src={section.src} width={500}/>
</figure>
...
<h3 className="font-semibold text-2xl text-center text-sky-500 mt-6 group-hover:text-gray-50  relative">{section.name}</h3>
<p className="text-center group-hover:text-gray-50  relative">{section.job}</p>
</header>

<ul className="flex justify-center space-x-4 mt-16 text-sky-500
relative">
...
<ToolTipsWebsite title={t('title')} viewDetaile={t('viewDetaile')}/>
